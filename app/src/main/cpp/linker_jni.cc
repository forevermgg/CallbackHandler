...
bool ScanRegionInBuffer(const char *buf, size_t length, uintptr_t *out_address,
                        size_t *out_size) {
  std::string buf_str(buf);
  LOG_ERROR("ScanRegionInBuffer buf = %s", buf_str.c_str());
  ...
  if (strcmp(permissions, "---p")) return false;

  if (vma_start % PAGE_SIZE || vma_end % PAGE_SIZE) return false;
...
bool AndroidDlopenExt(void *mapping_start, size_t mapping_size,
...
void NativeLibInfo::CloseRelroFd() {
  if (relro_fd_ == kInvalidFd) return;
  close(relro_fd_);
  ...
  for (int i = 0; i < ehdr->e_phnum; i++) {
    ...
    switch (phdr->p_type) {
      ...
      case PT_GNU_RELRO:
        ...
        if (min_relro_vaddr < min_vaddr) min_vaddr = min_relro_vaddr;
        if (max_vaddr < max_relro_vaddr) max_vaddr = max_relro_vaddr;
        break;
      default:
        break;
    }
...
Java_org_chromium_base_library_1loader_LinkerJni_nativeFindRegionReservedByWebViewZygote(
  ...
  uintptr_t address;
  size_t size;
  if (!FindWebViewReservation(&address, &size)) return false;
  s_lib_info_fields.SetLoadInfo(env, lib_info_obj, address, size);
  return true;
}
...
Java_org_chromium_base_library_1loader_LinkerJni_nativeUseRelros(
    JNIEnv *env, jclass clazz, jlong local_load_address,
